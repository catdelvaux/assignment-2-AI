...
class OurAgent(Agent):
 ...
 #   def evaluate_action(self, state, action):
        #for heuristic ordering function to optimize the choice → Move Ordering
 ...
 #       return self.evaluate(new_state)
    ...
    def max_value(self, state, alpha, beta, depth):
        ...
        for a in actions:
            if a in killers:
                killer_actions.append(a)
            else:
                normal_actions.append(a)
        actions = killer_actions+normal_actions
        ...
        for a in actions:
            ...
            if v >= beta:
                if depth not in self.killers:
                    self.killers[depth] = []
                if action not in self.killers[depth]:
                    self.killers[depth].insert(0, action)
